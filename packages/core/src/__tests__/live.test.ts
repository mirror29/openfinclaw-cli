...
import { executeStrategyLeaderboard } from "../strategy/tools.js";
...
describe.skipIf(!HAS_KEY)("Live API tests", () => {
  it("should fetch leaderboard from Hub API", async () => {
    ...
    const result = await executeStrategyLeaderboard({ boardType: "composite", limit: 5 }, config);
  ...
  it("should fetch leaderboard with different board types", async () => {
    const config = resolveConfigFromEnv();
    const result = await executeStrategyLeaderboard({ boardType: "returns", limit: 3 }, config);
